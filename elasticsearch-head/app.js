(function () {
    ...
    acx.vector.prototype = {
        ...
        mod: function (fn) {
            // runs a function against the x and y values
        ...
        within: function (p, q) {
            return this.x >= (p.x < q.x ? p.x : q.x) && this.x <= (p.x > q.x ? p.x : q.x) && this.y >= (p.y < q.y ? p.y : q.y) && this.y <= (p.y > q.y ? p.y : q.y);
        },
    ...
    ux.Class.extend = function (prop) {
        function Class() {
            if (!initializing) {
                ...
                this.config = $.extend(
                    (function (t) {
                        // automatically construct a config object based on defaults and last item passed into the constructor
...
(function (app) {
    ...
    ut.option_template = function (v) {
        return { tag: "OPTION", value: v, text: v };
    };

    ut.require_template = function (f) {
        return f.require ? { tag: "SPAN", cls: "require", text: "*" } : null;
    };
...
(function ($, app) {
    ...
    ux.DragDrop = ux.Observable.extend({
        ...
        pickup: function (jEv, opts) {
            ...
            if (!this.dragObj.get(0).parentNode || this.dragObj.get(0).parentNode.nodeType === 11) {
...
(function (app) {
    ...
    data.DataSourceInterface = ux.Observable.extend({
        ...
        _getSummary: function (res) {
            this.summary = i18n.text(
                "TableResults.Summary",
                res._shards.successful,
                res._shards.total,
                typeof res.hits.total === "object" ? res.hits.total.value : res.hits.total,
                (res.took / 1000).toFixed(3)
            );
        },
        ...
        getTypes: function (indices) {
            ...
            this.typesList.forEach(function (type) {
                for (var i = 0; i < indices.length; i++) {
                    if (!this.indices[indices[i]].types.contains(type)) return;
                }
                types.push(type);
            }, this);
            return types;
            ...
            function createField(mapping, index, type, path, name) {
                ...
                var field = (paths[dpath] =
                    ...
                    $.extend(
                        ...
                        default_property_map[coretype_map[mapping.type]],
                        mapping
                    ));
            ...
            this.config.cluster.get(
                ...
                function () {
                    ...
                    _cluster.get("_all", function (data) {
                        ...
                        for (var k in data) {
                            clusterState["routing_table"]["indices"][k] = {
                                shards: {
                                    1: [
                                        {
                                            state: "UNASSIGNED",
...
(function (app) {
    ...
    data.Query = ux.Observable.extend({
        ...
        setSort: function (index, desc) {
            var sortd = {};
            sortd[index] = { order: desc ? "asc" : "desc" };
    ...
    ui.AbstractWidget = ux.Observable.extend({
        ...
        init: function () {
            ...
            for (var prop in this) {
                // automatically bind all the event handlers
...
(function (app) {
    ...
    ui.PasswordField = ui.AbstractField.extend({
        init: function () {
            this._super();
        },
        _keyup_handler: function () {
    ...
    ui.CheckField = ui.AbstractField.extend({
        _main_template: function () {
            return {
                ...
                id: this.id(),
                cls: "uiCheckField",
                children: [{ tag: "INPUT", type: "checkbox", name: this.config.name, checked: !!this.config.value }],
            };
        },
        validate: function () {
...
(function ($, joey, app) {
    ...
    ui.Button = ui.AbstractWidget.extend({
        ...
        init: function (parent) {
            this._super();
            this.el = $.joey(this.button_template()).bind("click", this.click_handler);
        ...
        button_template: function () {
            return {
                ...
                children: [
                    {
                        tag: "DIV",
                        cls: "uiButton-content",
                        children: [{ tag: "DIV", cls: "uiButton-label", text: this.config.label }],
                    },
                ],
            };
        },
...
(function ($, app) {
    ...
    ui.SplitButton = ui.AbstractWidget.extend({
        ...
        _main_template: function () {
            return { tag: "DIV", cls: this._baseCls, children: [this.button, this.menuButton] };
        },
    });
})(this.jQuery, this.app);
                ...
                children: [
                    {
                        tag: "DIV",
                        cls: "pull-left",
                        children: [{ tag: "H2", text: this.config.label }].concat(this.config.left),
                    },
...
(function ($, app) {
    ...
    ui.AbstractPanel = ui.AbstractWidget.extend({
        ...
        shared: {
            // shared data for all instances of ui.Panel and decendants
            stack: [], // array of all open panels
            modal: $({
                tag: "DIV",
                id: "uiModal",
                css: { opacity: 0.2, position: "absolute", top: "0px", left: "0px" },
            }),
        },
        init: function () {
            this._super();
        ...
        open: function (ev) {
            this.el
                ...
                .css({
                    zIndex: this.shared.stack.length ? +this.shared.stack[this.shared.stack.length - 1].el.css("zIndex") + 10 : 100,
                })
        ...
        _getPosition: function () {
            return $(window)
                .vSize() // get the current viewport size
    ...
    ui.DraggablePanel = ui.AbstractPanel.extend({
        ...
        _body_template: function () {
            return {
                tag: "DIV",
                cls: "uiPanel-body",
                css: { height: this.config.height + (this.config.height === "auto" ? "" : "px") },
                children: [this.config.body],
            };
        },
        _title_template: function () {
            return { tag: "SPAN", cls: "uiPanel-title", text: this.config.title };
        },
        _main_template: function () {
            return {
                tag: "DIV",
                ...
                children: [
                    {
                        tag: "DIV",
                        cls: "uiPanel-titleBar",
                        children: [{ tag: "DIV", cls: "uiPanel-close", onclick: this._close_handler, text: "x" }, this.title],
                    },
                    this.body,
                ],
            };
    ...
    ui.DialogPanel = ui.DraggablePanel.extend({
        ...
        _actionsBar_template: function () {
            return {
                tag: "DIV",
                cls: "pull-right",
                children: [new app.ui.Button({ label: "Cancel", onclick: this._close_handler }), new app.ui.Button({ label: "OK", onclick: this._commit_handler })],
            };
        },
    });
})(this.app);
...
(function (app) {
    ...
    ui.MenuPanel = ui.AbstractPanel.extend({
        ...
        _menuItem_template: function (item) {
            ...
            return {
                tag: "LI",
                cls: "uiMenuPanel-item" + (item.disabled ? " disabled" : "") + (item.selected ? " selected" : ""),
...
(function ($, app) {
    ...
    ui.Table = ui.AbstractWidget.extend({
        ...
        _headerRow_template: function (columns) {
            return {
                tag: "TR",
                cls: "uiTable-header-row",
                children: columns.map(function (column) {
                    var dir = (this.config.store.sort.column === column && this.config.store.sort.dir) || "none";
                    return {
                        tag: "TH",
                        data: { column: column, dir: dir },
                        cls: "uiTable-header-cell" + (dir !== "none" ? " uiTable-sort" : ""),
                        children: [
                            {
                                tag: "DIV",
                                children: [
        ...
        _body_template: function (data, columns) {
            return {
                tag: "TABLE",
                children: [].concat(this._headerRow_template(columns)).concat(
                        ...
                        return {
                            ...
                            children: columns.map(function (column) {
                                return {
                                    tag: "TD",
                                    cls: "uiTable-cell",
                                    children: [{ tag: "DIV", text: (row[column] || "").toString() }],
                                };
                            }),
                        };
                    })
    ...
    ui.CSVTable = ui.AbstractWidget.extend({
        ...
        _main_template: function () {
            return {
                ...
                cls: this._baseCls,
                id: this.id(),
                children: [this._downloadButton, this._downloadLink, { tag: "PRE", text: this._csvText }],
            };
        },
...
(function ($, app) {
    ...
    ui.JsonPretty = ui.AbstractWidget.extend({
        ...
        pretty: {
            // from https://github.com/RyanAmos/Pretty-JSON/blob/master/pretty_json.js
...
(function ($, app) {
    ...
    ui.PanelForm = ui.AbstractWidget.extend({
        ...
        _field_template: function (field) {
            return {
                tag: "LABEL",
                cls: "uiPanelForm-field",
                children: [{ tag: "DIV", cls: "uiPanelForm-label", children: [field.label, ut.require_template(field)] }, field],
            };
        },
    });
})(this.jQuery, this.app);
...
(function ($, app, i18n) {
    ...
    ui.SidebarSection = ui.AbstractWidget.extend({
        ...
        _showSection_handler: function (ev) {
            var shown = $(ev.target)
                .closest(".uiSidebarSection")
                .toggleClass("shown")
                .children(".uiSidebarSection-body")
                .slideToggle(
                    200,
                    function () {
                        this.fire("animComplete", this);
                    }.bind(this)
                )
                .end()
                .hasClass("shown");
        ...
        _main_template: function () {
            return {
                tag: "DIV",
                cls: "uiSidebarSection",
                children: [
                    this.config.title && {
                        tag: "DIV",
                        cls: "uiSidebarSection-head",
                        onclick: this._showSection_handler,
                        children: [
                            this.config.title,
                            this.config.help && {
                                tag: "SPAN",
                                cls: "uiSidebarSection-help pull-right",
                                onclick: this._showHelp_handler,
                                text: i18n.text("General.HelpGlyph"),
                            },
                        ],
                    },
...
(function ($, app, i18n) {
    ...
    ui.QueryFilter = ui.AbstractWidget.extend({
        ...
        requestUpdate: function (jEv) {
            if (jEv && jEv.originalEvent) {
                // we only want to update on real user interaction not generated events
        ...
        _selectType_handler: function (jEv) {
            ...
            jEl.toggleClass("selected");
            var type = jEl.text(),
                selected = jEl.hasClass("selected");
        ...
        _openFilter_handler: function (section) {
            ...
            if (!section.loaded) {
                ...
                if (spec.core_type === "string") {
                    ...
                    section.body.append(
                        new ui.DateHistogram({
                            printEl: section.body.find("INPUT"),
        ...
        _dateFilterChange_handler: function (jEv) {
            ...
            jEl.siblings(".uiQueryFilter-rangeHintFrom").text(i18n.text("QueryFilter.DateRangeHint.from", range.start && new Date(range.start).toUTCString()));
            jEl.siblings(".uiQueryFilter-rangeHintTo").text(i18n.text("QueryFilter.DateRangeHint.to", range.end && new Date(range.end).toUTCString()));
            jEl.data("uqid", uqid);
            this.requestUpdate(jEv);
        },
        ...
        _main_template: function () {
            return {
                ...
                id: this.id(),
                cls: "uiQueryFilter",
                children: [this._aliasSelector_template(), this._indexSelector_template(), this._typesSelector_template(), this._filters_template()],
            };
        },
        _aliasSelector_template: function () {
            ...
            return {
                tag: "DIV",
                cls: "uiQueryFilter-section uiQueryFilter-aliases",
                children: [{ tag: "SELECT", onChange: this._selectAlias_handler, children: aliases.map(ut.option_template) }],
            };
        },
        _indexSelector_template: function () {
            ...
            return {
                tag: "DIV",
                cls: "uiQueryFilter-section uiQueryFilter-indices",
                children: [
                    { tag: "HEADER", text: i18n.text("QueryFilter-Header-Indices") },
                    {
                        tag: "DIV",
                        onClick: this._selectIndex_handler,
                        children: indices.map(function (name) {
        ...
        _typesSelector_template: function () {
            ...
            return {
                tag: "DIV",
                cls: "uiQueryFilter-section uiQueryFilter-types",
                children: [
                    { tag: "HEADER", text: i18n.text("QueryFilter-Header-Types") },
                    {
                        tag: "DIV",
                        onClick: this._selectType_handler,
                        children: types.map(function (name) {
        ...
        _filters_template: function () {
            ...
            var fields = Object.keys(_metadataFields)
                .sort()
                .filter(function (d) {
                    return _metadataFields[d].core_type !== undefined;
                });
            return {
                tag: "DIV",
                cls: "uiQueryFilter-section uiQueryFilter-filters",
                children: [
                    { tag: "HEADER", text: i18n.text("QueryFilter-Header-Fields") },
                    {
                        tag: "DIV",
                        children: fields.map(function (name) {
        ...
        _dateFilter_template: function (spec) {
            return {
                ...
                children: [
                    ...
                    {
                        tag: "PRE",
                        cls: "uiQueryFilter-rangeHintFrom",
                        text: i18n.text("QueryFilter.DateRangeHint.from", ""),
                    },
                    {
                        tag: "PRE",
                        cls: "uiQueryFilter-rangeHintTo",
        ...
        _booleanFilter_template: function (spec) {
            return {
                tag: "SELECT",
                data: { spec: spec },
                onChange: this._booleanFilterChange_handler,
        ...
        _multiFieldFilter_template: function (section, spec) {
            return {
                tag: "DIV",
                cls: "uiQueryFilter-subMultiFields",
                children: acx.eachMap(
                    spec.fields,
                    function (name, data) {
    ...
    ui.AnyRequest = ui.Page.extend({
            ...
            this.history = this.prefs.get("anyRequest-history") || [
                {
                    ...
                    query: JSON.stringify(this.config.query),
        ...
        _request_handler: function (ev) {
            ...
            if (ev) {
                ...
                for (var i = 0; i < this.history.length; i++) {
                    if (this.history[i].path === path && this.history[i].type === type && this.history[i].query === query && this.history[i].transform === transform) {
        ...
        _responseWriter_handler: function (data) {
            ...
            if (this.asGraphEl.attr("checked")) {
                ...
                raphael(this.outEl[0], w - 10, 300).g.barchart(10, 10, w - 20, 280, [data]);
            }
            if (this.asTableEl.attr("checked")) {
                try {
                    var store = new app.data.ResultDataSourceInterface();
                    this.outEl.append(
                        new app.ui.ResultTable({
                ...
                children: [
                    {
                        ...
                        children: [
                            new app.ui.SidebarSection({
                                ...
                                body: {
                                    tag: "UL",
                                    onclick: this._historyClick_handler,
                                    cls: "uiAnyRequest-history",
                                    children: this.history.map(this._historyItem_template, this),
                                },
                            }),
                            new app.ui.SidebarSection({
                                open: true,
                                    ...
                                    children: [
                                        {
                                            ...
                                            type: "text",
                                            name: "base_uri",
                                            value: this.config.cluster.config.base_uri,
                                        },
                                        { tag: "BR" },
                                        { tag: "INPUT", type: "text", name: "path", value: this.config.path },
                                        {
                                            tag: "SELECT",
                                            name: "method",
                                            children: ["POST", "GET", "PUT", "HEAD", "DELETE"].map(ut.option_template),
                                        },
                                        {
                                            tag: "TEXTAREA",
                                            name: "body",
                                            rows: 20,
                                            text: JSON.stringify(this.config.query),
                                        },
                                        {
                                            tag: "BUTTON",
                                            cls: "uiButton-content",
                                            css: { cssFloat: "right" },
                                            type: "button",
                                            children: [{ tag: "B", text: i18n.text("AnyRequest.Request") }],
                                            onclick: this._request_handler,
                                        },
                                        {
                                            tag: "BUTTON",
                                            cls: "uiButton-content",
                                            type: "button",
                                            text: i18n.text("AnyRequest.ValidateJSON"),
                                            onclick: this._validateJson_handler,
                                        },
                                        {
                                            tag: "LABEL",
                                            cls: "uiButton-label",
                                            children: [{ tag: "INPUT", type: "checkbox", name: "pretty" }, i18n.text("AnyRequest.Pretty")],
                                        },
                            ...
                            new app.ui.SidebarSection({
                                ...
                                body: {
                                    ...
                                    children: [
                                        {
                                            tag: "LABEL",
                                            children: [{ tag: "INPUT", type: "checkbox", checked: true, name: "asJson" }, i18n.text("AnyRequest.AsJson")],
                                        },
                                        { tag: "BR" },
                                        {
                                            tag: "LABEL",
                                            children: [{ tag: "INPUT", type: "checkbox", name: "asGraph" }, i18n.text("AnyRequest.AsGraph")],
                                        },
                                        { tag: "BR" },
                                        {
                                            tag: "LABEL",
                                            children: [{ tag: "INPUT", type: "checkbox", name: "asTable" }, i18n.text("AnyRequest.AsTable")],
                                        },
                                    ],
                                },
                            }),
        ...
        _historyItem_template: function (item) {
            return {
                ...
                cls: "booble",
                data: { item: item },
                children: [{ tag: "SPAN", text: item.path }, " ", { tag: "EM", text: item.query }, " ", { tag: "SPAN", text: item.transform }],
            };
        },
    });
...
(function (app, i18n, joey) {
    ...
    ui.NodesView = ui.AbstractWidget.extend({
        ...
        init: function () {
            ...
            this._styleSheetEl = joey({
                tag: "STYLE",
                text: ".uiNodesView-nullReplica, .uiNodesView-replica { zoom: " + this.config.scaleReplicas + " }",
            });
        ...
        _newAliasAction_handler: function (index) {
            var fields = new app.ux.FieldCollection({
                fields: [new ui.TextField({ label: i18n.text("AliasForm.AliasName"), name: "alias", require: true })],
            });
            var dialog = new ui.DialogPanel({
                ...
                onCommit: function (panel, args) {
                    if (fields.validate()) {
                        ...
                        var command = {
                            actions: [{ add: { index: index.name, alias: data["alias"] } }],
                        };
                        this.config.cluster.post(
                            "_aliases",
        ...
        _postIndexAction_handler: function (action, index, redraw) {
            this.cluster.post(
                encodeURIComponent(index.name) + "/" + encodeURIComponent(action),
                null,
                function (r) {
        ...
        _optimizeIndex_handler: function (index) {
            var fields = new app.ux.FieldCollection({
                fields: [
                    new ui.TextField({
                        label: i18n.text("OptimizeForm.MaxSegments"),
                        name: "max_num_segments",
                        value: "1",
                        require: true,
                    }),
                    new ui.CheckField({
                        label: i18n.text("OptimizeForm.ExpungeDeletes"),
                        name: "only_expunge_deletes",
                        value: false,
                    }),
                    new ui.CheckField({ label: i18n.text("OptimizeForm.FlushAfter"), name: "flush", value: true }),
                    new ui.CheckField({
                        label: i18n.text("OptimizeForm.WaitForMerge"),
                        name: "wait_for_merge",
                        value: false,
                    }),
                ],
        ...
        _forceMergeIndex_handler: function (index) {
            var fields = new app.ux.FieldCollection({
                fields: [
                    new ui.TextField({
                        label: i18n.text("ForceMergeForm.MaxSegments"),
                        name: "max_num_segments",
                        value: "1",
                        require: true,
                    }),
                    new ui.CheckField({
                        label: i18n.text("ForceMergeForm.ExpungeDeletes"),
                        name: "only_expunge_deletes",
                        value: false,
                    }),
        ...
        _shutdownNode_handler: function (node) {
            if (prompt(i18n.text("IndexCommand.ShutdownMessage", i18n.text("Command.SHUTDOWN"), node.cluster.name)) === i18n.text("Command.SHUTDOWN")) {
                this.cluster.post(
                    "_cluster/nodes/" + encodeURIComponent(node.name) + "/_shutdown",
                    null,
                    function (r) {
                        alert(JSON.stringify(r));
                        this.fire("redraw");
        ...
        _deleteAliasAction_handler: function (index, alias) {
            if (confirm(i18n.text("Command.DeleteAliasMessage", alias.name, index.name))) {
                var command = {
                    actions: [{ remove: { index: index.name, alias: alias.name } }],
                };
                this.config.cluster.post(
                    "_aliases",
                    JSON.stringify(command),
        ...
        _nodeControls_template: function (node) {
            return {
                tag: "DIV",
                cls: "uiNodesView-controls",
                children: [
                    new ui.MenuButton({
                        ...
                        menu: new ui.MenuPanel({
                            items: [
                                {
                                    text: i18n.text("NodeInfoMenu.ClusterNodeInfo"),
                                    onclick: function () {
                                        new ui.JsonPanel({ json: node.cluster, title: node.name });
                                    },
                                },
                                {
                                    text: i18n.text("NodeInfoMenu.NodeStats"),
                                    onclick: function () {
                                        new ui.JsonPanel({ json: node.stats, title: node.name });
                                    },
                                },
                            ],
                        }),
                    ...
                    new ui.MenuButton({
                        ...
                        menu: new ui.MenuPanel({
                            items: [
                                {
                                    text: i18n.text("NodeActionsMenu.Shutdown"),
                                    onclick: function () {
                                        this._shutdownNode_handler(node);
                                    }.bind(this),
                                },
                            ],
                        }),
        ...
        _nodeIcon_template: function (node) {
            ...
            return {
                tag: "TD",
                title: alt,
                cls: "uiNodesView-icon",
                children: [{ tag: "SPAN", cls: "fa fa-2x " + icon }],
            };
        },
        _node_template: function (node) {
            return {
                tag: "TR",
                cls: "uiNodesView-node" + (node.master_node ? " master" : ""),
                children: [
                    this._nodeIcon_template(node),
                    {
                        ...
                        children:
                            node.name === "Unassigned"
                                ...
                                : [{ tag: "H3", text: node.cluster.name }, { tag: "DIV", text: node.cluster.hostname }, this.interactive ? this._nodeControls_template(node) : null],
                    },
                ].concat(node.routings.map(this._routing_template, this)),
            };
        },
        _indexHeaderControls_template: function (index) {
            return {
                tag: "DIV",
                cls: "uiNodesView-controls",
                children: [
                    new ui.MenuButton({
                        ...
                        menu: new ui.MenuPanel({
                            items: [
                                {
                                    text: i18n.text("IndexInfoMenu.Status"),
                                    onclick: function () {
                                        new ui.JsonPanel({ json: index.status, title: index.name });
                                    },
                                },
                                {
                                    text: i18n.text("IndexInfoMenu.Metadata"),
                                    onclick: function () {
                                        new ui.JsonPanel({ json: index.metadata, title: index.name });
                                    },
                                },
                            ],
                        }),
                    ...
                    new ui.MenuButton({
                        ...
                        menu: new ui.MenuPanel({
                            items: [
                                {
                                    text: i18n.text("IndexActionsMenu.NewAlias"),
                                    onclick: function () {
                                        this._newAliasAction_handler(index);
                                    }.bind(this),
                                },
                                {
                                    text: i18n.text("IndexActionsMenu.Refresh"),
                                    onclick: function () {
                                        this._postIndexAction_handler("_refresh", index, false);
                                    }.bind(this),
                                },
                                {
                                    text: i18n.text("IndexActionsMenu.Flush"),
                                    onclick: function () {
                                        this._postIndexAction_handler("_flush", index, false);
                                    }.bind(this),
                                },
                                {
                                    text: this.cluster.versionAtLeast("5.0.0.") ? i18n.text("IndexActionsMenu.ForceMerge") : i18n.text("IndexActionsMenu.Optimize"),
                                    onclick: this.cluster.versionAtLeast("5.0.0.")
                                        ? function () {
                                              this._forceMergeIndex_handler(index);
                                          }.bind(this)
                                        : function () {
                                              this._optimizeIndex_handler(index);
                                          }.bind(this),
                                },
                                {
                                    text: i18n.text("IndexActionsMenu.Snapshot"),
                                    disabled: closed,
                                    onclick: function () {
                                        this._postIndexAction_handler("_gateway/snapshot", index, false);
                                    }.bind(this),
                                },
                                {
                                    text: i18n.text("IndexActionsMenu.Analyser"),
                                    onclick: function () {
                                        this._testAnalyser_handler(index);
                                    }.bind(this),
                                },
                                {
                                    text: index.state === "close" ? i18n.text("IndexActionsMenu.Open") : i18n.text("IndexActionsMenu.Close"),
                                    onclick: function () {
                                        this._postIndexAction_handler(index.state === "close" ? "_open" : "_close", index, true);
                                    }.bind(this),
                                },
                                {
                                    text: i18n.text("IndexActionsMenu.Delete"),
                                    onclick: function () {
                                        this._deleteIndexAction_handler(index);
                                    }.bind(this),
                                },
                            ],
                        }),
        ...
        _indexHeader_template: function (index) {
            ...
            var line1 = closed
                ? "index: close"
                : "size: " +
                  (index.status && index.status.primaries && index.status.total
                      ? ut.byteSize_template(index.status.primaries.store.size_in_bytes) + " (" + ut.byteSize_template(index.status.total.store.size_in_bytes) + ")"
                      : "unknown");
            var line2 = closed
                ? "\u00A0"
                : "docs: " +
                  (index.status && index.status.primaries && index.status.primaries.docs && index.status.total && index.status.total.docs
                      ? index.status.primaries.docs.count.toLocaleString() + " (" + (index.status.total.docs.count + index.status.total.docs.deleted).toLocaleString() + ")"
                      : "unknown");
            return index.name
                ? {
                      tag: "TH",
                      cls: closed ? "close" : "",
                      children: [{ tag: "H3", text: index.name }, { tag: "DIV", text: line1 }, { tag: "DIV", text: line2 }, this.interactive ? this._indexHeaderControls_template(index) : null],
                  }
                : [{ tag: "TD" }, { tag: "TH" }];
        },
            ...
            return (
                cluster.aliases.length && {
                    ...
                    children: [
                        {
                            tag: "TR",
                            children: [{ tag: "TD" }].concat(
                                indices.map(function (index, row) {
                                    var indexName = index.name;
                                    return {
                                        tag: "TD",
                                        children:
                                            index.metadata &&
                                            index.metadata.aliases.map(function (alias, index) {
                                                return {
                                                    tag: "DIV",
                                                    css: {
                                                        "margin-bottom": "1px",
                                                        background: "#" + "9ce9c7fc9".substr((row + 6) % 7, 3),
                                                    },
                                                    cls: "uiNodesView-hasAlias",
                                                    text: alias,
                                                    children:
                                                        true || this.interactive
                                                            ? [
                                                                  {
                                                                      tag: "SPAN",
        ...
        _aliasRender_template_full: function (cluster, indices) {
            return (
                cluster.aliases.length && {
                    tag: "TBODY",
                    cls: "alias-body",
                    children: cluster.aliases.map(function (alias, row) {
                        return {
                            tag: "TR",
                            children: [{ tag: "TD" }, { tag: "TD" }].concat(
                                alias.indices.map(function (index, i) {
                                    if (index) {
                                        return {
                                            tag: "TD",
        ...
        _main_template: function (cluster, indices) {
            return {
                tag: "TABLE",
                cls: "table uiNodesView",
                children: [
                    this._styleSheetEl,
                    {
                        tag: "THEAD",
                        children: [{ tag: "TR", children: indices.map(this._indexHeader_template, this) }],
                    },
    ...
    ui.ClusterOverview = ui.Page.extend({
        ...
        draw_handler: function () {
            ...
            function getIndexForIndex(routings, i) {
                var index = (indexIndices[i] =
                    i in indexIndices ? (routings[indexIndices[i]] = routings[indexIndices[i]] || newIndex(i)) && indexIndices[i] : (routings[indexIndicesIndex] = newIndex(i)) && indexIndicesIndex++);
                indices[index] = i;
                return index;
            }
            ...
            var aliasesIndex = {};
            var aliases = [];
            var indexClone = indices.map(function () {
                return false;
            });
            $.each(clusterState.metadata.indices, function (name, index) {
            ...
            cluster.nodes = nodes.filter(nodeFilter_none).sort(this._nodeSort);
        ...
        _main_template: function () {
            return {
                tag: "DIV",
                id: this.id(),
                cls: "uiClusterOverview",
                children: [
                    new ui.Toolbar({
                        label: "", //label: i18n.text("Overview.PageTitle"),
                        left: [this._nodeSortMenu, this._indicesSortMenu, this._aliasMenu, this._indexMenu, this._indexFilter],
                        right: [this._refreshButton],
                    }),
...
(function (app, i18n, raphael) {
    ...
    ui.DateHistogram = ui.AbstractWidget.extend({
        ...
        _stat_handler: function (query, results) {
            if (!results.aggregations[this.statAggs]) {
                return;
            }
        ...
        _aggs_handler: function (query, results) {
            if (!results.aggregations[this.dateAggs]) {
                return;
            }
            var buckets = [],
            ...
            r.g
                .barchart(0, 0, w, h, [buckets], { gutter: "0", vgutter: 0 })
                .hover(
                    function () {
                        this.flag = r.g.popup(this.bar.x, h - 5, this.value || "0").insertBefore(this);
                    },
                    function () {
                        this.flag.animate({ opacity: 0 }, 200, ">", function () {
                            this.remove();
                        });
                    }
                )
        ...
        _main_template: function () {
            return {
                tag: "DIV",
                cls: "uiDateHistogram loading",
                css: { height: "50px" },
                children: [i18n.text("General.LoadingAggs")],
            };
        },
    });
    ...
    ui.ClusterConnect = ui.AbstractWidget.extend({
        ...
        _node_handler: function (data) {
            if (data) {
                ...
                if (data.version && data.version.number) this.cluster.setVersion(data.version.number);
            }
        },
        ...
        _optionCluster_template: function (uri, name, version) {
            return {
                tag: "OPTION",
                value: uri,
                text: name + " (" + this._hideCredentialsInURI(uri) + ")" + (version ? " (" + version + ")" : ""),
            };
        },
        ...
        _promptConnect_handler: function () {
            var fields = new app.ux.FieldCollection({
                fields: [
                    new ui.TextField({ label: "URL (e.g. http://localhost:9200/)", name: "url", require: true }),
                    new ui.TextField({
                        label: "Username",
                        name: "username",
                    }),
                    new ui.PasswordField({
                        label: "Password",
                        name: "password",
                    }) /*,
                    new ui.TextField({
                ...
                onCommit: function (panel, args) {
                    if (fields.validate()) {
                        ...
                        if (data.username || data.password) {
                            urlObject.username = data.username || "";
            ...
            /*this.el
                .find(".uiClusterConnect-uri")
                .val(
                    prompt(
                        "Elasticsearch End-point URI? \n(e.g. http://username:password@localhost:9200)",
        ...
        _main_template: function () {
            return {
                tag: "SPAN",
                cls: "uiClusterConnect",
                children: [
                    {
                        tag: "BUTTON",
                        type: "button",
                        text: i18n.text("Header.NewConnect"),
                        onclick: this._promptConnect_handler,
                    },
                    {
                        tag: "INPUT",
                        type: "hidden",
                        cls: "uiClusterConnect-uri",
                        onkeyup: function (ev) {
                            if (ev.which === 13) {
                                ev.preventDefault();
                                this._reconnect_handler();
                        ...
                        id: this.id("baseUri"),
                        value: this.cluster.base_uri,
                    },
                    { tag: "SPAN", type: "span", cls: "uiClusterSelector-select" },
                    {
                        tag: "BUTTON",
                        type: "button",
                        text: i18n.text("Header.ServerListRemove"),
                        onclick: this._removeServer_handler,
                    },
                ],
            };
        },
    ...
    var StructuredQuery = ui.AbstractWidget.extend({
        ...
        _indexChanged_handler: function (index) {
            ...
            this.filter = new ui.FilterBrowser({
                cluster: this.config.cluster,
                index: index,
                onStartingSearch: function () {
                    this.el.find("DIV.uiStructuredQuery-out").text(i18n.text("General.Searching"));
                    this.el.find("DIV.uiStructuredQuery-src").hide();
                }.bind(this),
        ...
        _jsonResults_handler: function (results) {
            this.el
                .find("DIV.uiStructuredQuery-out")
                .empty()
                .append(new ui.JsonPretty({ obj: results }));
        },
        _csvResults_handler: function (results) {
            this.el
                .find("DIV.uiStructuredQuery-out")
                .empty()
                .append(new ui.CSVTable({ results: results }));
        },
        ...
        _searchSource_handler: function (src) {
            ...
            if (typeof JSON !== "undefined") {
                var showRawJSON = $({
                    tag: "BUTTON",
                    type: "button",
                    text: i18n.text("StructuredQuery.ShowRawJson"),
                    id: "showRawJSON",
                    value: JSON.stringify(src),
                    onclick: this._showRawJSON,
                });
    ...
    ui.FilterBrowser = ui.AbstractWidget.extend({
        ...
        init: function (parent) {
            ...
            new data.MetaDataFactory({
                ...
                onReady: function (metadata, eventData) {
                    this.metadata = metadata;
                ...
                if (obj.properties) {
                    ...
                    if (obj.fields) {
                        for (var subField in obj.fields) {
                            filters.push({
                                path: path[path.length - 1] !== subField ? path.concat(subField) : path,
                                type: obj.fields[subField].type,
                                meta: obj.fields[subField],
                            });
                        }
                    }
        ...
        _changeQueryField_handler: function (jEv) {
            ...
            } else if (spec.type === "long" || spec.type === "integer" || spec.type === "float" || spec.type === "byte" || spec.type === "short" || spec.type === "double") {
            ...
            select.after({
                tag: "SELECT",
                cls: "op",
                onchange: this._changeQueryOp_handler,
                children: ops.map(ut.option_template),
            });
            select.next().change();
        },
            ...
            return {
                ...
                children: [
                    { tag: "DIV", cls: "uiFilterBrowser-filters" },
                    {
                        ...
                        cls: "uiButton-content",
                        type: "button",
                        text: i18n.text("General.Search"),
                        onclick: this._search_handler,
                    },
                    {
                        tag: "LABEL",
                        cls: "uiButton-label",
                        children: i18n.complex("FilterBrowser.OutputType", {
                            tag: "SELECT",
                            cls: "uiFilterBrowser-outputFormat",
                            children: [
                                { text: i18n.text("Output.Table"), value: "table" },
                                { text: i18n.text("Output.JSON"), value: "json" },
                                { text: i18n.text("Output.CSV"), value: "csv" },
                            ].map(function (o) {
                                return $.extend({ tag: "OPTION" }, o);
                            }),
                        }),
                    },
                    {
                        tag: "LABEL",
                        children: i18n.complex("FilterBrowser.OutputSize", {
                            tag: "SELECT",
                            cls: "uiFilterBrowser-outputSize",
                            children: ["10", "50", "250", "1000", "5000", "25000"].map(ut.option_template),
                        }),
                    },
                    {
                        tag: "LABEL",
                        children: [{ tag: "INPUT", type: "checkbox", cls: "uiFilterBrowser-showSrc" }, i18n.text("Output.ShowSource")],
                    },
                ],
            };
        },
        ...
        _filter_template: function () {
            return {
                tag: "DIV",
                cls: "uiFilterBrowser-row",
                children: [
                    { tag: "SELECT", cls: "bool", children: ["must", "must_not", "should"].map(ut.option_template) },
                    {
                        tag: "SELECT",
                        cls: "field",
                        onchange: this._changeQueryField_handler,
                        children: this.filters.map(function (f) {
                            return {
                                tag: "OPTION",
                                data: { spec: f },
                                value: f.path.join("."),
            ...
            return {
                ...
                children: [
                    { tag: "INPUT", cls: "qual", type: "text" },
                    {
                        tag: "SELECT",
                        cls: "fuzzyop",
                        children: ["max_expansions", "min_similarity"].map(ut.option_template),
                    },
        ...
        init: function () {
            ...
            this._clusterState.on(
                ...
                function (state) {
                    ...
                    this.statEl.text(i18n.text("Header.ClusterHealth", colour, shards.successful, shards.total)).css("background", colour);
                }.bind(this)
            );
        ...
        _main_template: function () {
            return {
                ...
                children: [
                    ...
                    { tag: "SPAN", cls: "pull-right", children: [this._quickMenu] },
...
(function ($, app, i18n) {
    ...
    ui.IndexOverview = ui.Page.extend({
        ...
        _refresh_handler: function () {
            ...
            var view = {
                indices: acx
                    .eachMap(state.status.indices, function (name, index) {
                        return {
                            name: name,
                            state: index,
                        };
            ...
            var dialog = new ui.DialogPanel({
                ...
                onCommit: function (panel, args) {
                    if (fields.validate()) {
                        ...
                        var name = data["_name"];
                        delete data["_name"];
                        this.config.cluster.put(
                            encodeURIComponent(name),
                            JSON.stringify({ settings: { index: data } }),
                            function (d) {
                                dialog.close();
                                alert(JSON.stringify(d));
                                this._clusterState.refresh();
        ...
        _indexTable_template: function (view) {
            return {
                tag: "TABLE",
                cls: "table table-padding",
                children: [
                    {
                        tag: "THEAD",
                        children: [
                            {
                                ...
                                children: [
                                    ...
                                    { tag: "TH", children: [{ tag: "STRONG", text: "Size" }] },
                                    { tag: "TH", children: [{ tag: "STRONG", text: "Docs" }] },

                                    { tag: "TH", children: [{ tag: "STRONG", text: "Deleted" }] },
                                    { tag: "TH", children: [{ tag: "STRONG", text: "#Primary Shard" }] },
                                    { tag: "TH", children: [{ tag: "STRONG", text: "#Replica Shard" }] },
                                ],
                            },
                        ],
                    },
        ...
        _index_template: function (index) {
            return {
                ...
                children: [
                    ...
                    {
                        tag: "TD",
                        text: ut.byteSize_template(index.state.primaries.store.size_in_bytes) + "/" + ut.byteSize_template(index.state.total.store.size_in_bytes),
                    },
...
(function (app, i18n) {
    ...
    app.App = ui.AbstractWidget.extend({
        ...
        _openAnyRequest_handler: function (ev) {
            this.navigateTo("AnyRequest", { cluster: this.cluster }, ev);
        },
        _openStructuredQuery_handler: function (ev) {
            this.navigateTo("StructuredQuery", { cluster: this.cluster }, ev);
        },
        _openBrowser_handler: function (ev) {
            this.navigateTo("Browser", { cluster: this.cluster }, ev);
        },
        _openClusterOverview_handler: function (ev) {
            this.navigateTo("ClusterOverview", { cluster: this.cluster, clusterState: this._clusterState }, ev);
        },
        _openIndexOverview_handler: function (ev) {
            this.navigateTo("IndexOverview", { cluster: this.cluster, clusterState: this._clusterState }, ev);
        },

        _body_template: function () {
            return { tag: "DIV", id: this.id("body"), cls: "uiApp-body" };
        },
                ...
                children: [
                    {
                        ...
                        children: [
                            ...
                            {
                                tag: "DIV",
                                cls: "uiApp-headerMenu",
                                children: [
                                    {
                                        tag: "DIV",
                                        cls: "uiApp-headerMenuItem pull-left",
                                        text: i18n.text("Nav.Overview"),
                                        onclick: this._openClusterOverview_handler,
                                    },
                                    {
                                        tag: "DIV",
                                        cls: "uiApp-headerMenuItem pull-left",
                                        text: i18n.text("Nav.Indices"),
                                        onclick: this._openIndexOverview_handler,
                                    },
                                    {
                                        tag: "DIV",
                                        cls: "uiApp-headerMenuItem pull-left",
                                        text: i18n.text("Nav.Browser"),
                                        onclick: this._openBrowser_handler,
                                    },
                                    {
                                        tag: "DIV",
                                        cls: "uiApp-headerMenuItem pull-left",
                                        text: i18n.text("Nav.StructuredQuery"),
                                        onclick: this._openStructuredQuery_handler,
                                        children: [{ tag: "A", cls: "uiApp-headerNewMenuItem ", text: " [+]" }],
                                    },
                                    {
                                        tag: "DIV",
                                        cls: "uiApp-headerMenuItem pull-left",
                                        text: i18n.text("Nav.AnyRequest"),
                                        onclick: this._openAnyRequest_handler,
                                        children: [{ tag: "A", cls: "uiApp-headerNewMenuItem ", text: " [+]" }],
                                    },
                                ],
                            },
